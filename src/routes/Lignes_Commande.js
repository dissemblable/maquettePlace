...
export const Lignes_CommandesRouter = (db) =>
  ...
  .get("/:id", async (req, res) => {
    try {
      const [results] = await db.query(`SELECT * FROM Lignes_Commandes WHERE id = ?`,
        [req.params.id]
      );
      res.json(results);
    } catch (err) {
  ...
  .post("/", async (req, res) => {
    ...
    try {
      const [results] = await db.query(`INSERT INTO Lignes_Commandes (nom, commande_id, produit_id, quantite, prix_unitaire) VALUES (?, ?, ?, ?, ?)`,
        [nom, commande_id, produit_id, quantite, prix_unitaire]
      );
      res.status(201).json({ id: results.insertId, nom, commande_id, produit_id, quantite, prix_unitaire });
    } catch (err) {
  ...
  .put("/:id", async (req, res) => {
    const { nom, commande_id, produit_id, quantite, prix_unitaire } = req.body;
    try {
      const [results] = await db.query(`UPDATE Lignes_Commandes SET nom = ?, commande_id = ?, produit_id = ?, quantite = ?, prix_unitaire = ? WHERE id = ?`,
        [nom, commande_id, produit_id, quantite, prix_unitaire, req.params.id]
      );
    } catch (err) {
  ...
  .delete("/:id", async (req, res) => {
    try {
      const [results] = await db.query(`DELETE FROM Lignes_Commandes WHERE id = ?`,
        [req.params.id]
      );
    } catch (err) {
